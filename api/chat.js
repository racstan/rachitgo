    ...
    if (provider === "GEMINI") {
      ...
      switch (provider) {
        ...
        case "OPENROUTER":
          ...
          model = process.env.OPENROUTER_MODEL || process.env.AI_MODEL || "google/gemini-2.0-flash-lite-001";
          break;
        case "MISTRAL":
      ...
      if (provider === "OPENROUTER") {
        requestHeaders["HTTP-Referer"] = "https://rachitgo.dev";
        requestHeaders["X-Title"] = "Rachit Asthana Portfolio AI";
      }
      ...
      const data = await response.json();
      let rawReply = data?.choices?.[0]?.message?.content || `No reply from ${provider}`;
      // Clean up thinking/reasoning blocks if generated by models like DeepSeek R1
      reply = rawReply.replace(/<think>[\s\S]*?<\/think>/gi, "").trim() || rawReply;
    }
